...
wpgpDownloadFileFromFTP <- function(file_path, dest_file, username, password, quiet) {
  ...
  tryCatch(
    {
      utils::download.file(file_remote, destfile=dest_file,mode="wb",quiet=quiet)
    },
    error=function(cond) {
...
#' @importFrom utils read.csv
wpgpGetCSVFileAllCovariates <- function(username, password) {
...
#' @return Dataframe
#' @export
wpgpListCountries <- function(username, password, verbose=FALSE) {
...
wpgpListCountryCovariates <- function(ISO3=NULL, username=NULL, password=NULL, detailed=FALSE) {
...
wpgpGetCountryCovariate <- function(ISO3=NULL,
  ...
  if (nrow(df.filtered)!=1){
    stop( paste0("Entered Covariates: ",covariate," does not present in WP"))
  }
  ...
  wpgpDownloadFileFromFTP(file_remote, file_local, username, password, quiet=quiet)
